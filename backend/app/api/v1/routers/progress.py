...
@user_progress_router.get("/{lesson_id}", response_model=UserProgress)
async def get_user_steps_progress(
    lesson_id: int,
    service: UserProgressServiceDep,
    user: User = Depends(current_user),
):
    progress = await service.get_user_step_progress(lesson_id, user)
    return progress


@user_progress_router.get("/", response_model=UserProgress)
